#include "XDigitalLifeServerApp/MainFrm.h"
#include "stdafx.h"
#include "CFtpd.h"
#include "FtpdPi.h"
//
...
void CFtpdPi::ProcessMsg(char *cmd)
{
	...
	if(!bLOGINOK)
	{
		...
		}else if(strncmp("PASS ",curCmd,5) == 0 && bUSER == true)//密码
		{
			char password[256] = "";
	...
	{
		if(stricmp("XPWD",curCmd) == 0 || stricmp("PWD",curCmd) == 0)//返回当前目录
		{
		...
		else if(strncmp("CDUP",curCmd,4) == 0)//返回到父目录
		{
			if(goParent(vCurrentDir))
				wsprintf(str,"250 \"%s\" is current directory.%s",vCurrentDir,CRLF);
			else
		...
		else if(strcmp("REIN",curCmd) == 0)//终止一个用户
		{
			bUSER = false;
			bPASS = false;
			bLOGINOK = false;
		...
		else if(strncmp("DELE ",curCmd,5) == 0)//删除文件
		{
			char path[512] = "";
			strcpy(path,RemoveLeadingSpace(curCmd+4));
			if(vRemove(path))
		...
		else if(strncmp("XMKD ",curCmd,5) == 0)//创建目录
		{
			char path[512] = "";
		...
		else if(strcmp("ABOR",curCmd) == 0)//中止上一次FTP命令
		{
			try
			{
				dbsocket->Close();
		...
		else if(strncmp("STOR ",curCmd,5) == 0 || strncmp("STOU ",curCmd,5) == 0)//让服务器接受数据
		{
			CFile* outFile;
			try
			{
		...
		else if(strncmp("APPE ",curCmd,5) == 0)//附加和创建文件
		{
			CFile* outFile;
			try
			{
		...
		else if(strncmp("MKD ",curCmd,4) == 0)//创建目录
		{
			char path[512] = "";
			strcpy(path,RemoveLeadingSpace(curCmd+4));
			if(vMkdir(path))
		...
		else if(strncmp("RMD ",curCmd,4) == 0)//删除目录
		{
			char path[512] = "";
			strcpy(path,RemoveLeadingSpace(curCmd+4));
			if(vRmdir(path))
		...
		else if(strncmp("TYPE ",curCmd,5) == 0)//确定数据传输方式
		{
			char path[8] = "";
			strcpy(path,RemoveLeadingSpace(curCmd+5));
			if(strcmp("I",path)==0)
		...
		else if(stricmp("SYST",curCmd) == 0)//查看服务器类型
		{
		...
		else if(stricmp("QUIT",curCmd) == 0)//退出
		{
		...
		else if(stricmp("REST ",curCmd) == 0)//断点设置
		{
			char marker[32] = "";
		...
		else if(strncmp("PORT ",curCmd,5) == 0)//端口
		{
			char path[32] = "";
			int pHi=0;
			int pLo=0;
...
{
    //::AfxMessageBox(user);
	return !strcmp(user,buf);
	
}

bool CFtpdPi::matchpass(char *buf, char *uname)
{
    return !strcmp(pass,buf);
	//return true;
}
...
void CFtpdPi::ListEngine(char *vpath, bool longList)//建立目录并发送
{
	char path[256] = "";
	HANDLE Hfound;
	try
...
void CFtpdPi::SetpMain(CMainFrame *pMain)
{
	
	pMainWnd = pMain;
}

void CFtpdPi::OnNewFile(CString filename)
{
	pMainWnd->PostMessage(WM_NEW_FILE,0,0);
}
